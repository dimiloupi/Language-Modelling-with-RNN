...
class RNN(object):
	...
	def predict(self, x):
		...
		for t in range(len(x)):
			...
			# --- your code here --- #
			##########################
			net_in = (self.V.dot(make_onehot(x[t], self.vocab_size )) + self.U.dot(s[t-1]))
			s[t] = sigmoid(net_in)
	...
	def acc_deltas(self, x, d, y, s):
		...
		for t in reversed(range(len(x))):
			...
			d_y = d_onehot - y[t]
			gnet_out = np.ones(len(d_onehot))
			delta_out = np.multiply(d_y, gnet_out)
			DW = np.outer(delta_out, s[t])
			self.deltaW = np.add(DW, self.deltaW)
			gradient = grad(s[t])
			delta_in = (np.matmul(self.W.T, delta_out) * gradient)
			DV_outer = np.outer(delta_in, x_onehot)
			#self.deltaV += DV_outer
			self.deltaV = np.add(DV_outer, self.deltaV)

			DU_outer = np.outer(delta_in, s[t-1])
			self.deltaU = np.add(self.deltaU, DU_outer)
	...
	def acc_deltas_bptt(self, x, d, y, s, steps):
		...
		for t in reversed(range(len(x))):
			...
			for taf in range(1, steps+1):
				if t-taf >= 0:
					delta_in = np.dot(self.U.T, delta_in) * grad(s[t-taf])
					taf_onehot = make_onehot(x[t-taf], self.vocab_size)
					self.deltaV += np.outer(delta_in, taf_onehot)
					self.deltaU += np.outer(delta_in, s[t-taf-1])
	...
	def compute_loss(self, x, d):
		...
		##########################
		y, s = self.predict(x)
		correct=np.zeros(d.shape[0])
		for i in range(0,d.shape[0]):
			correct[i]= np.log(y[i, d[i]])
	...
	def generate_sequence(self, start, end, maxLength):
		...
		while sequence[-1] != end and len(sequence) < maxLength:
			if len(sequence) == 1:
				y,s = self.predict(x)
				samples_items = multinomial_sample(y[-1])
				sequence.append(samples_items)
				loss += -1 * np.log(y[-1][samples_items])
			else:
				y,s = self.predict(sequence)
				samples_items = multinomial_sample(y[-1])
				sequence.append(samples_items)
				loss += -1 * np.log(y[-1][samples_items])
...
if __name__ == "__main__":
	...
	if mode == "estimate":
		...
		for hidden_layer in [25,50]:
			for tau in [0, 2, 5]:
				for lr in [0.1, 0.5, 0.05]:
					r = RNN(vocab_size, hidden_layer)
					error = r.train(X=X, D=D, X_dev=X_dev, D_dev=D_dev, epochs=10, anneal=5, learning_rate=lr, back_steps=tau, batch_size=100, min_change=0.0001, log=True)
					if error < best_loss:
						best_loss = error
						best_hdim = hidden_layer
						best_lookback = tau
						best_lr = lr


		print("\nestimation finished.\n\tbest loss {0}:\tbest hidden/lookback/learn rate: {1}/{2}/{3}".format(best_loss, best_hdim, best_lookback, best_lr))
	...
	if mode == "train":
		...
		# --- your code here --- #
		##########################
		all_xdev, all_Ddev = seqs_to_lmXY(S_dev)
		run_loss = -1
		adjusted_loss = -1

		r = RNN(vocab_size, hdim)
		x = r.train(X=X_train, D=D_train, X_dev=X_dev, D_dev=D_dev, epochs=10, learning_rate=lr, back_steps=lookback)
		np.save("rnn.U.npy", r.U)
		np.save("rnn.V.npy", r.V)
		np.save("rnn.W.py", r.W)
		run_loss = r.compute_mean_loss(all_xdev, all_Ddev)
		adjusted_loss = adjust_loss(run_loss, fraction_lost, q, mode = 'basic')

		print "The mean loss is %.03f" %run_loss
	...
	if mode == "generate":
		...
		for i in range(10):
			sent=[]
			sequence, loss = r.generate_sequence(start, end, maxLength)

			for word in sequence:
				sent.append(num_to_word[word])
				mean_loss = loss/len(sequence)
			print " ".join(sent)
			print "Loss: %.03f" % loss
			print "Mean Loss: %.03f" % (mean_loss)
			print "Perplexity: %.03f" % np.exp(loss)
			print "Perplexity of Mean Loss: %.03f" % np.exp(mean_loss)
